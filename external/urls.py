...
urlpatterns = [
    ...
    url(r'^receiveFile/(?P<username>[0-9a-zA-Z@\.\+\-\_]{0,30})/(?P<userkey>[A-Z0-9]{256})/(?P<file_name>[0-9a-zA-Z@\.\+\-\_" "]{0,260})/(?P<encrypted>((True)|(False)))$', receiveFile, name='receiveFile'),
    #See the requested report
    url(r'^seeReport/(?P<username>[0-9a-zA-Z@\.\+\-\_]{0,30})/(?P<report_name>[a-zA-Z0-9@\.\+\-\_" "]{0,50})$', seeReport, name='seeReport'),
    #Get the requested file
    url(r'^getFile/(?P<username>[0-9a-zA-Z@\.\+\-\_]{0,30})/(?P<file_name>[0-9a-zA-Z0-9@\.\+\-\_\-" "]{0,50})$', getFile, name='getFile'),
]